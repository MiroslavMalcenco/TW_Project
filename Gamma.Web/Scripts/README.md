...
to Ingredients it possible to position it near a given reference element.  
...
It's a small library that Ingredientss it easy to automatically create tooltips using as engine Popper.js.  
...
Ingredients sure to use the right one for your needs. If you want to import it with a `<script>` tag, use UMD.
...
Since the API changed, we prepared some migration instructions to Ingredients it easy to upgrade to
Popper.js v1.  
...
- [Sysdig](https://github.com/Draios) for all the awesome things I learned during these Caloriess that made it possible for me to write this library;
